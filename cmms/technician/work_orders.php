...
// Build query for work orders - ONLY FOR THIS TECHNICIAN
...
if (!empty($status_filter)) {
    switch ($status_filter) {
        case 'active':
            $query .= " AND m.Started_Date IS NOT NULL AND m.Completed_Date IS NULL";
            break;
        case 'pending':
            $query .= " AND m.Started_Date IS NULL AND m.Completed_Date IS NULL";
            break;
        case 'completed':
...
<head>
    ...
    <title>Ordres de Travail - Technicien | CMMS Hôpital</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
        ...
        <main class="main-content">
            ...
            <div class="stats-grid">
                <div class="stat-mini">
                    ...
                    <div class="stat-mini-value" id="pending-count">0</div>
                </div>
                <div class="stat-mini">
                ...
                <div class="stat-mini">
                    <div class="stat-mini-label">Complétées</div>
                    <div class="stat-mini-value" id="completed-count">0</div>
                </div>
                <div class="stat-mini">
            ...
            <div class="content-section">
                <div class="section-header">
                    <h2>Liste des Ordres de Travail</h2>
                    <span style="font-size: 14px; color: #999;"><?php echo count($work_orders); ?> résultat(s)</span>
                </div>
                        ...
                        <tbody>
                            <?php if (!empty($work_orders)): ?>
                                <?php foreach ($work_orders as $order): ?>
                                    <tr>
                                        <td><strong>#<?php echo $order['Maintenance_ID']; ?></strong></td>
                                        <td><?php echo htmlspecialchars($order['Asset_Name']); ?></td>
                                        <td><?php echo htmlspecialchars($order['Type_Name']); ?></td>
                                        <td>
                                            <span class="priority-<?php echo strtolower($order['Priority_Name']); ?>">
                                                <?php echo htmlspecialchars($order['Priority_Name']); ?>
                                            </span>
                                        </td>
                                        <td><?php echo date('Y-m-d', strtotime($order['Reported_Date'])); ?></td>
                                        <td><?php echo $order['Scheduled_Date'] ? date('Y-m-d', strtotime($order['Scheduled_Date'])) : 'N/A'; ?></td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="work_order_detail.php?id=<?php echo $order['Maintenance_ID']; ?>" class="action-btn action-view">👁️ Voir</a>
                                            <?php if ($order['status_class'] === 'pending'): ?>
                                                <a href="update_order.php?id=<?php echo $order['Maintenance_ID']; ?>&action=start" class="action-btn action-start">▶️ Démarrer</a>
                                            <?php elseif ($order['status_class'] === 'active'): ?>
                                                <a href="update_order.php?id=<?php echo $order['Maintenance_ID']; ?>&action=complete" class="action-btn action-complete">✓ Compléter</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="8" class="no-data">Aucun ordre de travail trouvé</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            rows.forEach(row => {
                if (row.textContent.includes('En Attente')) pending++;
                if (row.textContent.includes('En Cours')) active++;
                if (row.textContent.includes('Complétée')) completed++;
            });
